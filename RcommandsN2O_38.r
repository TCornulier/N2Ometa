...
# for use later:
NCEP.deriv.vars<- c("WetDays.prop.year", "FTCy.year", "meanPosDDays.air.year", "meanPosDDays.soil.year", "FrostDDays.soil.year", "WetDays.exp", "WetDays.exp6", "WetDays.exp10", "SumPrecip.exp", "SumPrecip.exp.halfNorm80", "DegDays.exp.halfNorm80", "DegDays.exp", "DegDays.Soil.exp", "FTCy.exp", "PosDDays.air.exp", "PosDDays.soil.exp", "FrostDDays.soil.exp", "AvDegDays.exp", "AvPrecip.exp", "PropWetDays.exp")
...
######################## loading data ########################
dat<- read.delim("N2O_Database_TEMPLATE_working document_02Mar2018.txt", na.string= c("#N/A", "", "NA"), skip= 0, sep= "\t", strip.white= T, stringsAsFactors= T)
# dat<- dat[, 1:83]
...
NO3.table<- 		read.delim("NO3avail_lookup_09Apr19_T.txt", stringsAsFactors= T)
precrop.4.table<- 	read.delim("Tables for regrouping_ULE_2017-7-14_sheet2precrop.txt", stringsAsFactors= T)
Main.Crop.6.table<- read.delim("Tables for regrouping_ULE_2017-7-14_sheet3maincrop.txt", stringsAsFactors= T)
N2O.Method.table<- 	read.delim("Tables for regrouping_ULE_2017-7-14_sheet4method.txt", stringsAsFactors= T)
Nuptake.table<-		read.delim("Nuptake_groups_ULE.txt", stringsAsFactors= T)

# save.image("DataImport20210127.RData")
...
NO3.table[, 1:3]<- 		DF.Capitalize(NO3.table[, 1:3], cols.sub= c(T, T, T))          
...
dat<- merge(dat[, -excl], NO3.table, by= "Fertilizer.type_rep", sort= F, all.x= T)
dat[is.na(dat$Fert.type.rep) & (!is.na(dat$Pub.N)), c("Fert.type.GROUP", "NO3.avail")]<- NO3.table["NA", ] # from Frank: "NA for Fertilizer.type_rep stems from two papers, which did not specify the type of mineral fertilizer used. However, since it is mineral N we have assumed a "Mineral_N_mix". I think we can keep it"

# # simplify NO3.avail further (redundant from version 37)
...
precip.threshold<- 6 # was 1 prior to v38
...
for(i in extent$studyID[NCEP.nonempty]){
	...
	extent$WetDays.prop.year[extent$studyID == i]<- sum((NCEP.list[[i]]$prate.sfc * 24 * 60 * 60) > precip.threshold, na.rm= T)/(Nyr*365)
}
...
## Assuming wet day is > 6 mm
extent$WetDays.exp6<- NA
for(i in extent$studyID[NCEP.nonempty]){
	exp.extent<- NCEP.list[[i]]$Date.POSIXct > extent$START.day[extent$studyID == i] & NCEP.list[[i]]$Date.POSIXct < extent$END.day[extent$studyID == i]
	extent$WetDays.exp6[extent$studyID == i]<- sum((NCEP.list[[i]]$prate.sfc[exp.extent] * 24 * 60 * 60) > 6, na.rm= T)
}
plot(extent$WetDays.exp6)

## Assuming wet day is > 10 mm
extent$WetDays.exp10<- NA
for(i in extent$studyID[NCEP.nonempty]){
	exp.extent<- NCEP.list[[i]]$Date.POSIXct > extent$START.day[extent$studyID == i] & NCEP.list[[i]]$Date.POSIXct < extent$END.day[extent$studyID == i]
	extent$WetDays.exp10[extent$studyID == i]<- sum((NCEP.list[[i]]$prate.sfc[exp.extent] * 24 * 60 * 60) > 10, na.rm= T)
}
plot(extent$WetDays.exp10)
...
pairs(extent[, c("WetDays.exp", "WetDays.exp6", "WetDays.exp10")], asp= 1)
plot(extent$WetDays.exp6 / extent$WetDays.exp)
plot(extent$WetDays.exp10 / extent$WetDays.exp)
...
# save(extent, extent.all, file= "Data_Clim_20210127.RData")
# load("Data_Clim_20210127.RData")
...
dat$logWetDays.exp6<- log(dat$WetDays.exp6+1)
dat$logWetDays.exp6.compact<- log(dat$WetDays.exp6+3) - 3 # mean(dat$logWetDays.exp6.compact, na.rm= T) # 3.22

dat$logWetDays.exp10<- log(dat$WetDays.exp10+1)
dat$logWetDays.exp10.compact<- log(dat$WetDays.exp10+1) - 2.5 # mean(dat$logWetDays.exp6.compact, na.rm= T) # 2.54

par(mfrow= c(1, 3))
hist(dat$WetDays.exp10, nclass= 25)
hist(dat$logWetDays.exp10, nclass= 25)
hist(dat$logWetDays.exp10.compact, nclass= 25)
...
tmp<- unique(dat[, c("NO3.avail", "NO3.avail.v2", "NO3.avail.v3", "NO3.avail.f3", "NO3.avail.f4")])
...
dat$NO3.3cat.low<- as.numeric(dat$NO3.avail.f4 == "lowNO3supply")
dat$NO3.3cat.med<- as.numeric(dat$NO3.avail.f4 == "mediumNO3supply")
dat$NO3.3cat.high<- as.numeric(dat$NO3.avail.f4 == "highNO3supply")

dat$NO3.avail.f3<- factor(dat$NO3.avail.f3, levels= c("noFert", "lowNO3", "highNO3", "Undefined"))
dat$NO3.avail.f4<- factor(dat$NO3.avail.f4, levels= c("noNfert", "lowNO3supply", "mediumNO3supply", "highNO3supply"))

dat$NO3.avail.f4.num<- as.numeric(dat$NO3.avail.f4)

p1<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f4, nrow= 1, drop= F)+
	xlab("logWetDays (> 1mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
	theme_gray() + 
	scale_fill_gradient(low= muted("green", c= 10, l= 100), high= muted("green", c= 80, l= 25), trans= "log10")
# p1

p2<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f3, nrow= 1, drop= F)+
	xlab("logWetDays (> 1mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
	theme_gray() + 
	scale_fill_gradient(low= muted("green", c= 10, l= 100), high= muted("green", c= 80, l= 25), trans= "log10")
...
p3<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp10.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f4, nrow= 1, drop= F)+
	xlab("logWetDays (> 10mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
	theme_gray() + 
	scale_fill_gradient(low= muted("green", c= 10, l= 100), high= muted("green", c= 80, l= 25), trans= "log10")
# p3

p4<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp10.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f3, nrow= 1, drop= F)+
	xlab("logWetDays (> 10mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
...
p5<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp6.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f4, nrow= 1, drop= F)+
	xlab("logWetDays (> 6mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
	theme_gray() + 
	scale_fill_gradient(low= muted("green", c= 10, l= 100), high= muted("green", c= 80, l= 25), trans= "log10")

p6<- ggplot(dat, aes(y= logclay.imp.ct, x= logWetDays.exp6.compact)) +
	geom_hex(bins= 7) +
	facet_wrap(~ NO3.avail.f3, nrow= 1, drop= F)+
	xlab("logWetDays (> 6mm)") + ylab("logClay") +
	geom_point(alpha= 1, size= 1) + 
...
g2<- arrangeGrob(p3, p4, nrow= 2)
g3<- arrangeGrob(p5, p6, nrow= 2)
gm<- marrangeGrob(list(g1, g2, g3), nrow= 1, ncol= 1)
ggsave("Explo_2020_03_24.pdf", gm, width= 30, height= 20, units= "cm")

table(dat$NO3.avail.f3)
table(dat$NO3.avail.f4)

par(mfrow= c(2, 4))
sub<- dat$Fert01 == 0
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp.compact[sub], xlab= "logClay", ylab= "logWetDays (>1mm)")
sub<- dat$NO3.3cat.low == 1
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp.compact[sub], xlab= "logClay", ylab= "logWetDays (>1mm)")
sub<- dat$NO3.3cat.med == 1
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp.compact[sub], xlab= "logClay", ylab= "logWetDays (>1mm)")
sub<- dat$NO3.3cat.high == 1
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp.compact[sub], xlab= "logClay", ylab= "logWetDays (>1mm)")
...
par(mfrow= c(2, 4))
sub<- dat$Fert01 == 0
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp10.compact[sub], xlab= "logClay", ylab= "logWetDays (>10mm)")
sub<- dat$NO3.3cat.low == 1
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp10.compact[sub], xlab= "logClay", ylab= "logWetDays (>10mm)")
sub<- dat$NO3.3cat.med == 1
plot(dat$logclay.imp.ct[sub], dat$logWetDays.exp10.compact[sub], xlab= "logClay", ylab= "logWetDays (>10mm)")
...
# save formatted data
# save(dat, file= "Data_full_20210127.RData")
## load("Data_full_20180312.RData") # used for M9
# load("Data_full_20210127.RData")
...
write.xlsx(dat, "ForInfoCompleteDataExport20210127.xlsx")
write.xlsx(dat, "ForInfoCompleteDataExport20210127_includingRICE.xlsx")
...
M15c.vars<- c("logN2O.cum.pos", "pH.cent", "Grasslands", "logNrate.scaled", "logclay.imp.ct", "Fert01", "NO3.avail.v3", "studyID.f", "logDegDays.exp.compact", "NO3.avail.f3", "logSumPrecip.exp.compact", "logWetDays.exp6.compact", "logCarbon", "logNrate.scaled.lowNO3", "logNrate.scaled.highNO3", "highNO3") #, "PosDDays.air.exp", "FrostDDays.soil.exp", "PosDDays.soil.exp", "Nuptake.group.num", "logFTCy.exp", "logAvPrecip.exp.ct", "AvDegDays.exp.ct"
sub.M15c<- apply(dat[, c("logN2O.cum.pos", "logNrate.cent.v2", "Fert01", "NO3.avail.v3", "studyID.f", "logWetDays.exp6.compact", "logclay.imp")], 1, FUN= function(x) !any(is.na(x))) # leave only variables for which NAs cannot be imputed, so that obs with missing data are removed # , "Bulk.density.imputed"
...
pregam.M15c<- jagam(formula= logN2O.cum.pos ~ Grasslands
    ...
    # + highNO3
 	+ logCarbon
	+ s(pH.cent, bs= "cr", k= 9)
	+ te(logclay.imp.ct, logWetDays.exp6.compact, k= c(5, 5))
	+ te(logclay.imp.ct, logWetDays.exp6.compact, k= c(5, 5), by= logNrate.scaled.lowNO3)
	+ te(logclay.imp.ct, logWetDays.exp6.compact, k= c(5, 5), by= logNrate.scaled.highNO3)
...
hist(extent[extent$DURATION.days > 363 & extent$DURATION.days <367, "WetDays.exp6"]) # subset climate data for 365 days
range(extent[extent$DURATION.days > 363 & extent$DURATION.days <367, "WetDays.exp6"], na.rm= T) # subset climate data for 365 days # range ~ [30, 200]
...
df.M15c.complete<- function(x){ # function to compute transformed variables for prediction
    ...
    x$logWetDays.exp6.compact<- log(x$WetDays.exp6+3) - 3
...
ND.M15c.Nrate<- data.frame(Fert01= 1, lowNO3= 1, highNO3= 0, Grasslands = FALSE, 
	...
	WetDays.exp6= 60, DegDays.exp= 3600, # @ Germany over 1 year
...
# Soil (Clay) * Wetness (WetDays.exp6) interaction # baseline is Clay= 25% and WetDays.exp6= 0 days
...
for(i in c(50, 150, 500)){
    ...
    ND.M15c.baseWATERLOG<- data.frame(Fert01= 0, lowNO3= 0, highNO3= 0, Grasslands = FALSE, 
        ...
        expand.grid(Clay= seq(25, 25, l= N.sq), WetDays.exp6= rep(0, l= N.sq)), studyID.f= " 32.58 119.702007 8  8120082007-08-152007-11-04") # some random studyID.f (the first one)
    ND.M15c.baseWATERLOG<- df.M15c.complete(ND.M15c.baseWATERLOG)

    sq.clay<- seq(0, 60, l= N.sq)
    sq.WetDays<- seq(0, 100, l= N.sq)

    ND.M15c.WATERLOG.common<- data.frame(Fert01= 0, lowNO3= 0, highNO3= 0, Grasslands = FALSE, 
        SOC= 2, pH= 7, # SOC @ median, SOC @ 2%
        N.rate= N.rate.p, DegDays.exp= 3600, # @ Germany over 1 year
        expand.grid(Clay= sq.clay, WetDays.exp6= sq.WetDays), studyID.f= " 32.58 119.702007 8  8120082007-08-152007-11-04") # some random studyID.f (the first one)
    ...
    round(res, 3)
    sel<- dat$WetDays.exp6 < 100
    points(trans3d(jitter(dat$clay[sel], 2), jitter(dat$WetDays.exp6[sel], 6), min(rng), pmat = res), col = rgb(0, 0, 0, 0.3), pch = 16, cex= 0.4)
    par(new= T)
...
pdf("GAM.M15coutputBayes_splines_Fig3_SE_restricted.pdf", width= 10, height= 8)
	...
	mtext("c)", 3, 3, adj= 0)
	mtext("Effect of N rate (Kg, log-scale)", 2, 3)
